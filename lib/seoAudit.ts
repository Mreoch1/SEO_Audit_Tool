...
// Helper functions moved to audit-helpers.ts
...
export async function runAudit(
  ...
  // DEBUG: Check if rootUrl is valid
  try {
    new URL(rootUrl)
    console.log('[Audit] Root URL is valid')
  } catch (e) {
    console.error('[Audit] Root URL is invalid:', rootUrl)
  }
...
async function analyzePage(url: string, userAgent: string, needsImageDetails = false, isMainPage = false): Promise<PageData> {
  try {
    ...
    if (isMainPage) {
      console.log(`[Audit Progress] ✅ isMainPage=true, fetching PageSpeed Insights for main page (this may take 60+ seconds)...`)
      console.log(`[Audit Progress] PageSpeed API key check: ${process.env.PAGESPEED_INSIGHTS_API_KEY ? '✅ Set' : '❌ Missing'}`)
    } else {
      console.log(`[Audit Progress] ⚠️ isMainPage=false, skipping PageSpeed for: ${url}`)
    }
    ...
    if (isConnectionError) {
      ...
      try {
        ...
        // CRITICAL FIX: Ensure isMainPage flag is preserved in retry path
        if (isMainPage) {
          console.log(`[Audit Progress] ✅ Retry path: isMainPage=true, fetching PageSpeed Insights for main page...`)
        }
        const pageSpeedPromise = isMainPage ? fetchPageSpeedInsights(url) : Promise.resolve(null)
        const pageData = await parseHtmlWithRenderer(
          rendered.renderedHtml,
        ...
        if (pageSpeedData) {
          ...
          // CRITICAL FIX: Save full PageSpeedData structure in retry path too, not just simplified metrics
          // This ensures we can access both mobile and desktop data later
          const serializablePageSpeedData = {
            mobile: {
              lcp: pageSpeedData.mobile.lcp,
    ...
    // CRITICAL FIX: Still fetch PageSpeed for main page even in fallback path
    let pageSpeedPromise: Promise<any> = Promise.resolve(null)
    if (isMainPage) {
      console.log(`[Audit Progress] ✅ Fallback path: isMainPage=true, fetching PageSpeed Insights for main page...`)
      const { fetchPageSpeedInsights } = await import('./pagespeed')
      pageSpeedPromise = fetchPageSpeedInsights(url)
    }
    ...
    // CRITICAL FIX: Wait for PageSpeed data and add it to fallback data
    const pageSpeedData = await pageSpeedPromise
    if (pageSpeedData) {
      console.log(`[PageSpeed] ✅ Successfully retrieved PageSpeed data for ${url} (fallback path)`)
      const serializablePageSpeedData = {
        mobile: {
          lcp: pageSpeedData.mobile.lcp,
          fcp: pageSpeedData.mobile.fcp,
      ...
      fallbackData.pageSpeedData = serializablePageSpeedData as any
      console.log(`[PageSpeed] ✅ Saved PageSpeed data to fallback pageData: mobile.lcp=${serializablePageSpeedData.mobile.lcp}, desktop.lcp=${serializablePageSpeedData.desktop.lcp}`)
    } else if (isMainPage) {
      console.warn(`[PageSpeed] ⚠️ No PageSpeed data available for ${url} (fallback path). Check API key, quota, or network connection.`)
    }
...
function analyzeSiteWideIssues(
  ...
  titleMap.forEach((pageEntries, title) => {
    if (pageEntries.length > 1) {
      ...
      // CRITICAL FIX: Only add URLs that aren't already in duplicateTitles to avoid double-counting
      // This ensures siteWide.duplicateTitles contains unique URLs only
      const existingUrls = new Set(siteWide.duplicateTitles)
      const newUrls = urls.filter(url => !existingUrls.has(url))
      
      if (isTemplateBased) {
        // Template-based: Report once with context (but still count in siteWide)
        siteWide.duplicateTitles.push(...newUrls)
        issues.push({
          category: 'On-page',
